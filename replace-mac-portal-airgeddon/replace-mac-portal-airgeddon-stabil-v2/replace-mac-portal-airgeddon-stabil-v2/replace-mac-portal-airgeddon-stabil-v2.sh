...
# -----------------------------#
#  List folder target (hanya nama folder)
# -----------------------------#
...
for F in "${FILES[@]}"; do
    DIR=$(dirname "$F")
    FOLDER_NAME=$(basename "$DIR")  # Ambil hanya nama folder
    if [[ -z "${FOLDER_MAP[$DIR]}" ]]; then
        FOLDER_MAP[$DIR]=1
        echo -e " ${GREEN}$NUM.${NC} $FOLDER_NAME"
...
# Tampilkan path lengkap setelah dipilih
echo -e "\n${GREEN}✔ Folder dipilih:${NC} ${BLUE}$TARGET_DIR${NC}"
